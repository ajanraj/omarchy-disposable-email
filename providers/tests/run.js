...
// Each field-shape rule of the normalizer guards, rejected and accepted
assert.equal(SimpleLogin.normalizeMailbox(null), null)
assert.equal(SimpleLogin.normalizeMailbox("orders@example.com"), null)
assert.equal(SimpleLogin.normalizeMailbox({ id: 0, email: "a@b.co" }), null)
assert.equal(SimpleLogin.normalizeMailbox({ id: 1 }), null)
assert.equal(SimpleLogin.normalizeMailbox({ id: 1, email: "" }), null)
assert.equal(SimpleLogin.normalizeMailbox({ id: "7", email: "a@b.co" }).id, 7)
assert.equal(SimpleLogin.normalizeSuffix(null), null)
assert.equal(SimpleLogin.normalizeSuffix({ suffix: ".cat@example.com" }), null)
assert.equal(SimpleLogin.normalizeSuffix({ signed_suffix: "", suffix: ".cat@example.com" }), null)
assert.equal(SimpleLogin.normalizeSuffix({ signed_suffix: "signed" }), null)
assert.equal(SimpleLogin.normalizeSuffix({ signed_suffix: "signed", suffix: "" }).label, "")
assert.equal(SimpleLogin.normalizeAlias(null), null)
assert.equal(SimpleLogin.normalizeAlias({ id: "x", email: "a@b.co" }), null)
assert.equal(SimpleLogin.normalizeAlias({ id: 1 }), null)
assert.equal(SimpleLogin.normalizeAlias({ id: 1, email: "" }), null)
assert.equal(SimpleLogin.normalizeAlias({ id: 1, email: "a@b.co" }).counters, "0 fwd, 0 replied, 0 blocked")
